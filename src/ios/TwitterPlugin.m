...
- (void) composeTweet:(CDVInvokedUrlCommand*)command
{
    ...
    if(!ok){
        ...
        [super.viewController presentViewController:tweetViewController animated:YES completion:nil];
    }
        ...
        if(granted) {
            ...
            if([accountsArray count] > 0) {
                ...
                [request performRequestWithHandler:^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error) {
                    ...
                    if([urlResponse statusCode] == 200) {
                        NSString *dataString = [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding];
                       // NSLog(NSString *dataString);
                     //   NSDictionary *dict = [dataString objectFromJSONString];
                        jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:dataString] toSuccessCallbackString:callbackId];
                        [dataString release];
                    }
                    else{
                        jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
